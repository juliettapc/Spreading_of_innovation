...
#remember!!! every time that i wanna read the csv file, i must read it AGAIN!!
    ...
    for row in results:       
        try:          
            ...
            date=str(row[2])          ##  OJO!!!!!! REVISAR EL NUMERO DE ESTA COLUMNA PQ PUEDE SER DISTINTO DE UN ARCHIVO DE DATOS A OTROS       
            parts=date.split(" ")[0]  
            parts=parts.split("/")
            ...
            attending=str(row[0]).split(" ")[0].strip(",").capitalize() #attenging in charge of the shift
            dr_who_ordered=str(row[1]).split(" ")[0].strip(",").capitalize() #actual doctor ordering procalcitonin 
            ...
            for key in sorted(dicc_days_cumul_num_orders):      # cumulative counts                
                if key >=  date_order:                          
                    ...
                    if dr_who_ordered in list_Fellows_ICU:
                        dicc_days_cumul_num_ICU_orders[key]+=1
