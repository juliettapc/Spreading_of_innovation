...
Created by Julia Poncela, on November 2012.
...
def main(graph_name):
   ...
   damping=1.0     #its harder to go back from YES to NO again. =1 means no effect, =0.5 half the movement from Y->N than the other way around, =0 never go back from Y to N
   ...
   basic_reeding_start_day=20   # and then plus minus a smaill random number 
   ...
   min_bump=0.0001  # for the doctors that are re-seeded 
   max_bump=0.3
   delta_bump=0.01
   near_threshold="YES"   # to reseed people near/far the threshold
...
##################################################################
   ...
   output_file2="../Results/Final_distance_vs_bump"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_unif_distr_"+str(Niter)+"iter_reseeding_start"+str(basic_reeding_start_day)+"r_near_threshold"+str(near_threshold)+".dat"        
   ...
   while bump <= max_bump:
    ...
    output_file=dir+"Time_evolutions_Persuasion_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_unif_distr_"+str(Niter)+"iter_reseeding_start"+str(basic_reeding_start_day)+"r_bump"+str(bump)+"_near_threshold"+str(near_threshold)+".dat"                
...
#       print start_reseeding 
       ...
       while t<= max_shift:  # loop over shifts, in chronological order  (the order is the day index since seeding_day) 
         ...
         if flag_future =="YES"  and t>= start_reseeding  and t==next_reseeding_day:
            reseeding(G,t,bump,threshold,num_reseeds,near_threshold)
   ...
   ##############end loop Niter
    ...
    print >> file2, bump, numpy.mean(list_ending_distances),numpy.std(list_ending_distances), float(tot_number_interventions)/float(Niter) ,float(tot_number_successful_interventions)/float(Niter),float(tot_number_succesfully_reseeded)/float(Niter)
    file2.close()
...
def  reseeding(G, t,bump,threshold,num_reseeds,near_threshold):
      ...
      if G.node[n]['type']=="shift" and (G.node[n]['order']>=t  and G.node[n]['order']<=t+7) and cont_reseeds<= num_reseeds:  # i look for the shift corresponding to that time step          
         ...
         for doctor in G.neighbors(n):    # for now, i give a bump to all doctors in the reseeding shift
            ...
            if G.node[doctor]["status"]=="NonAdopter": 
               ...
               if  near_threshold=="YES":

                  if G.node[doctor]["adoption_vector"]>=threshold/2.:  # dr near the threshold
                     G.node[doctor]["adoption_vector"]+=bump
                  else: pass
              
               else: # dr near the threshold
                  if G.node[doctor]["adoption_vector"]<threshold/2.:  # dr near the threshold
                     G.node[doctor]["adoption_vector"]+=bump
                  else: pass
