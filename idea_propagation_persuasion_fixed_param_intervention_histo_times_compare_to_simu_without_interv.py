...
def main(graph_name):
   ...
   time_window_ahead=5   # number of days in which there will be no Adopters on call
   ...
   min_bump=0.00  # for the doctors that are re-seeded 
   max_bump=0.65  #same scale as the status, and the adoption threshold
   delta_bump=0.2
...
######################################################################################
#  I read the file of the simulation of persuasion without intervention:            ##
######################################################################################


   filename_actual_evol="../Results/weight_shifts/persuasion/Time_evolutions_Persuasion_training_alpha"+str(alpha_A)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_unif_distr_1000iter_alphaA_eq_alphaF_243_A_F_inferred_middle.dat"
...
##################################################################
   ...
   output_file2="../Results/weight_shifts/Final_distance_vs_bump_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_num_reseed_per_shift"+str(num_reseeds)+"_"+str(Niter)+"iter_intervention_start"+str(basic_intervention_start_day)+"_window"+str(time_window_ahead)+"_comparison_to_simu_without_interv.dat"        
   file2 = open(output_file2,'wt')    
   file2.close()
   

   output_file5="../Results/weight_shifts/Landscape_intervention_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_num_reseed_per_shift"+str(num_reseeds)+"_"+str(Niter)+"iter_start"+str(basic_intervention_start_day)+"_window"+str(time_window_ahead)+"_comparison_to_simu_without_interv.dat"        
   file5 = open(output_file5,'wt')    
   file5.close()
   

   list_end_values_bump0=[]
   list_end_values_bump04=[]
   list_end_values_bump06=[]
   ...
   while bump <= max_bump:
    ...
    list_distances_100day_tot=[]
    list_distances_100day_Att=[]
    list_distances_100day_F=[]
    ...
    for iter in range(Niter):
       ...
       t=0#int(seed_shift)+1          
      
       while t<= 242:#max_shift:  # loop over shifts, in chronological order  (the order is the day index since seeding_day) 
        # print t,  time_evol_number_adopters_Att
         ...
         if flag_future =="YES"  and t>= start_intervention  and t==next_intervention_day:
          if bump >0.0:
            flag_tot,flag_Att,flag_F=intervention(G,t,bump,threshold,num_reseeds,list_intervention_days)
            
               #  print "intervention time!", t, list_intervention_days
...
#            print   "flags:",flag_tot, flag_Att, flag_F, "next interv:",next_intervention_day,"tot interv so far:",tot_number_interventions,len(list_intervention_days)
       ...
       list_distances_100day_Att.append(time_evol_number_adopters_Att[-143]-list_actual_evol[-143])  # because last day is 243      
       list_distances_100day_tot.append(time_evol_number_tot_adopters[-143]-list_actual_evol[-143])


       list_distances_150day_Att.append(time_evol_number_adopters_Att[-93]-list_actual_evol[-93])  
       ...
       ###### to prove whether or not the final number of adopters with/without intervention are different
       if  bump ==0.0:
          list_end_values_bump0.append( time_evol_number_tot_adopters[-1])       
       elif  bump ==0.4:
            list_end_values_bump04.append( time_evol_number_tot_adopters[-1])
       elif  bump ==0.6:
            list_end_values_bump06.append( time_evol_number_tot_adopters[-1])
   ...
   # print time_evol_number_adopters_Att_ITER
    ...
    print >> file2, bump, bump/float(threshold), numpy.mean(list_distances_100day_tot), numpy.mean(list_distances_100day_Att),numpy.mean(list_distances_150day_tot),numpy.mean(list_distances_150day_Att), numpy.mean(list_distances_200day_tot), numpy.mean(list_distances_200day_Att),numpy.mean(list_distances_at_end_tot), numpy.mean(list_distances_at_end_Att) , float(tot_number_interventions)/float(Niter) , float(tot_number_interventions_Att)/float(Niter) ,float(tot_number_interventions_F)/float(Niter) ,fraction_success_interv, fraction_success_interv_Att, fraction_success_interv_F
    file2.close()
...
#       print "list interv days:",list_intervention_days
   ...
   print "comparing distribution final distances"

   print  " for bump=0 ",numpy.mean(list_end_values_bump0),numpy.std(list_end_values_bump0)  
   print " bump=0.4",numpy.mean(list_end_values_bump04),numpy.std(list_end_values_bump04)
   print " bump=0.6",numpy.mean(list_end_values_bump06),numpy.std(list_end_values_bump06)

   
   print "ks: bump=0 vs bump=0.4:\n", stats.ks_2samp(list_end_values_bump0, list_end_values_bump04)  # compares distributions

   print "ks: bump=0 vs bump=0.6:\n", stats.ks_2samp(list_end_values_bump0, list_end_values_bump06)


  # print "\nt-test: bump=0 vs bump=0.4:\n",stats.ttest_ind(list_end_values_bump0, list_end_values_bump04)  # compares means
  # print "t-test: bump=0 vs bump=0.6:\n",stats.ttest_ind(list_end_values_bump0, list_end_values_bump06)



   histograma_gral_negv_posit.histograma(list_end_values_bump0,"../Results/weight_shifts/histogr_final_number_adopters_bump0_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_num_reseed_per_shift"+str(num_reseeds)+"_"+str(Niter)+"iter_intervention_start"+str(basic_intervention_start_day)+"_window"+str(time_window_ahead)+".dat", False)
   histograma_gral_negv_posit.histograma(list_end_values_bump04,"../Results/weight_shifts/histogr_final_number_adopters_bump0.4_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_num_reseed_per_shift"+str(num_reseeds)+"_"+str(Niter)+"iter_intervention_start"+str(basic_intervention_start_day)+"_window"+str(time_window_ahead)+".dat", False)
   histograma_gral_negv_posit.histograma(list_end_values_bump06,"../Results/weight_shifts/histogr_final_number_adopters_bump0.6_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_num_reseed_per_shift"+str(num_reseeds)+"_"+str(Niter)+"iter_intervention_start"+str(basic_intervention_start_day)+"_window"+str(time_window_ahead)+".dat", False)
...
def persuasion(G,damping,doctor1,doctor2,alpha_A,alpha_F,threshold,shift_length):
    ...
    if G.node[doctor1]["adoption_vector"]>=threshold or G.node[doctor2]["adoption_vector"]>=threshold :  # only if at least one doctor is Adopter
        ...
        alpha_A=alpha_A*shift_length   # to take into account the lenght shift
...
def  intervention(G, t,bump,threshold,num_reseeds,list_intervention_days):
    ...
    for n in G.nodes():     
      ...
      if G.node[n]['type']=="shift" :
         ...
         lenght=int(G.node[n]['shift_length'])
...
#                  print "i bumpped exactly what was necessary on shift", t
                  break
