...
# it is a dictionary: {0: (205.0, 146.0), 1: (168.0, 339.0), 3: (105.0, 372.0), ...}
    ...
    for n in H.nodes():
        if G.node[n]["type"]=="A":                     
            ...
            x_s=len(list_F)+float(G.node[n]["order"])#x_s+1000.0/float(len(list_s))
...
# diff types of doctors=diff colors      
    nx.draw_networkx_nodes(H,position_modified,nodelist=list_s, node_shape = 's', node_color='g', node_size=5) 
    nx.draw_networkx_nodes(H,position_modified,nodelist=list_F, node_shape = 'o', node_color='b', node_size=75)   
    nx.draw_networkx_nodes(H,position_modified,nodelist=list_A, node_shape = 'o', node_color='r', node_size=75)
