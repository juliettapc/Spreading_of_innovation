...
def main(graph_name):
    ...
    while alpha_F<= alpha_F_max:
        ...
        dir="../Results/"   
        output_file2=dir+"Final_number_Adopters_Persuasion_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_unif_distr_Noise"+str(noise)+"_"+str(Niter)+"_iter.dat"    
        ...
        for num_initial_shifts in range(tot_num_shifts):                                 
            ...
            output_file=dir+"Time_evolutions_Persuasion_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_num_ini_shifts"+str(num_initial_shifts)+"_unif_distr_Noise_"+str(noise)+"_"+str(Niter)+"iter.dat"        
                            ...
                            if flag_possible_persuasion==1:
                                ...
                                for pair in pairs:
                                    ...
                                    if G.node[doctor1]['status'] != G.node[doctor2]['status']:  # if they think differently, 
                                        ...
                                        persuasion(G,damping,doctor1,doctor2,alpha_A,alpha_F,noise,threshold)                                   
...
def persuasion(G,damping,doctor1,doctor2,alpha_A,alpha_F,noise,threshold):
  ...
  #print "  before:",G.node[doctor1]["label"],G.node[doctor1]["adoption_vector"],G.node[doctor1]['status'],",",G.node[doctor2]["label"],G.node[doctor2]["adoption_vector"],G.node[doctor2]['status']
    ...
    if (G.node[doctor1]['type']=="A") and (G.node[doctor2]['type']=="F"):
        ...
        if float(delta_AF)>=0.0:
            G.node[doctor1]["adoption_vector"]=G.node[doctor1]["adoption_vector"]-delta_AF*alpha_A*damping +random.uniform(-1.0,1.0)*noise  #from YES to NO
            G.node[doctor2]["adoption_vector"]=G.node[doctor2]["adoption_vector"]+delta_AF*alpha_F +random.uniform(-1.0,1.0)*noise  #from NO to YES
        else:
            G.node[doctor1]["adoption_vector"]=G.node[doctor1]["adoption_vector"]-delta_AF*alpha_A +random.uniform(-1.0,1.0)*noise
            G.node[doctor2]["adoption_vector"]=G.node[doctor2]["adoption_vector"]+delta_AF*alpha_F*damping +random.uniform(-1.0,1.0)*noise
        ...
        if float(delta_FA)>=0.0:
            G.node[doctor1]["adoption_vector"]=G.node[doctor1]["adoption_vector"]-delta_FA*alpha_F*damping +random.uniform(-1.0,1.0)*noise 
            G.node[doctor2]["adoption_vector"]=G.node[doctor2]["adoption_vector"]+delta_FA*alpha_A +random.uniform(-1.0,1.0)*noise
        else:
            G.node[doctor1]["adoption_vector"]=G.node[doctor1]["adoption_vector"]-delta_FA*alpha_F +random.uniform(-1.0,1.0)*noise
            G.node[doctor2]["adoption_vector"]=G.node[doctor2]["adoption_vector"]+delta_FA*alpha_A*damping +random.uniform(-1.0,1.0)*noise
        ...
        if float(delta_FF)>=0.0:
            G.node[doctor1]["adoption_vector"]=G.node[doctor1]["adoption_vector"]-delta_FF*alpha_F*damping +random.uniform(-1.0,1.0)*noise
            G.node[doctor2]["adoption_vector"]=G.node[doctor2]["adoption_vector"]+delta_FF*alpha_F +random.uniform(-1.0,1.0)*noise
        else:
            G.node[doctor1]["adoption_vector"]=G.node[doctor1]["adoption_vector"]-delta_FF*alpha_F +random.uniform(-1.0,1.0)*noise
            G.node[doctor2]["adoption_vector"]=G.node[doctor2]["adoption_vector"]+delta_FF*alpha_F*damping +random.uniform(-1.0,1.0)*noise
        ...
        if float(delta_AA)>=0.0:
            G.node[doctor1]["adoption_vector"]=G.node[doctor1]["adoption_vector"]-delta_AA*alpha_A*damping +random.uniform(-1.0,1.0)*noise
            G.node[doctor2]["adoption_vector"]=G.node[doctor2]["adoption_vector"]+delta_AA*alpha_A +random.uniform(-1.0,1.0)*noise
        else:
            G.node[doctor1]["adoption_vector"]=G.node[doctor1]["adoption_vector"]-delta_AA*alpha_A +random.uniform(-1.0,1.0)*noise
            G.node[doctor2]["adoption_vector"]=G.node[doctor2]["adoption_vector"]+delta_AA*alpha_A*damping +random.uniform(-1.0,1.0)*noise
