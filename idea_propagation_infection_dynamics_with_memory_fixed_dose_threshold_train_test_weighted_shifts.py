...
def main(graph_name):
   ...
   fixed_param="FIXED_Pimm0_"   #FIXED_Pimm0_"    # or ""  # for the Results file that contains the sorted list of best parameters
...
#####
      ...
      while prob_infection<= prob_max:
         ...
         while infect_threshold <= infect_threshold_max:
            ...
            while dose <= dose_max:
               ...
               output_file2=dir+"Average_time_evolution_Infection_memory_training_p"+str(prob_infection)+"_"+"Immune"+str(prob_Immune)+"_threshold"+str(infect_threshold)+"_dose"+str(dose)+"_"+str(Niter)+"iter_day"+str(cutting_day)+".dat"
            ...
            ########### set I.C.
               ...
               if (numpy.mean(list_dist_abs_at_ending_point_fixed_parameters)) <= delta_end:  # i only consider situations close enough at the ending point   
                  ...
                  histogram_filename="../Results/weight_shifts/histogr_raw_distances_ending_infection_memory_training_p"+str(prob_infection)+"_"+"Immune"+str(prob_Immune)+"_threshold"+str(infect_threshold)+"_dose"+str(dose)+"_"+str(Niter)+"iter_day"+str(cutting_day)+".dat"
                  histograma_gral_negv_posit.histograma(list_dist_at_ending_point_fixed_parameters,histogram_filename)
                  
                  histogram_filename2="../Results/weight_shifts/histogr_sum_dist_traject_infection_memory_training_p"+str(prob_infection)+"_"+"Immune"+str(prob_Immune)+"_threshold"+str(infect_threshold)+"_dose"+str(dose)+"_"+str(Niter)+"iter_day"+str(cutting_day)+".dat"
   ...
   list_order_dict= compare_real_evol_vs_simus_to_be_called.pick_minimum_same_end(dict_filenames_tot_distance,"Infection_memory_training",all_team,Niter,cutting_day)
# it returns a list of tuples like this :  ('../Results/network_final_schedule_withTeam3_local/infection/Average_time_evolution_Infection_training_p0.7_Immune0.0_2iter_2012.dat', [2540.0, 208.0, 1.0])  the best set of parameters  being the fist one of the elements in that list.
   ...
   string_name="infection_memory_training_"+fixed_param+str(Niter)+"iter_day"+str(cutting_day)+".dat"   # for the "Results" file with the sorted list of files
   
   list_order_dict2= compare_real_evol_vs_simus_to_be_called.pick_minimum_prod_distances(dict_filenames_prod_distances,string_name,all_team,Niter,cutting_day)
   ...
   infect_threshold=float(list_order_dict[0][0].split("_threshold")[1][0:3])
   dose=float(list_order_dict[0][0].split("_dose")[1][0:3])
 
  
   
  
   print "Optimum parameters (old method) at day",cutting_day," are: p=",prob_infection," Pimmune=",prob_Immune," infection threshold=",infect_threshold, " and dose=",dose
   


  # optimum_filename=list_order_dict2[0][0]

   prob_infection=float(list_order_dict2[0][0].split("_p")[1][0:3])
   prob_Immune=float(list_order_dict2[0][0].split("_Immune")[1][0:3])
   infect_threshold=float(list_order_dict2[0][0].split("_threshold")[1][0:3])
   dose=float(list_order_dict2[0][0].split("_dose")[1][0:3])
 
  
 
   print "Optimum parameters (product of distances and SDs) at day",cutting_day," are: p=",prob_infection," Pimmune=",prob_Immune," infection threshold=",infect_threshold, " and dose=",dose
