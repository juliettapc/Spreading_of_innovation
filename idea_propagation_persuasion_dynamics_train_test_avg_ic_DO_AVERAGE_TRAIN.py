...
def main(graph_name, Niter_training):
      ...
      while alpha_F<= alpha_F_max:            # i explore all the parameter space, and create a file per each set of values
        ...
        while  mutual_encouragement <= max_mutual_encouragement:
          ...
          while   damping <= max_damping:
           ...
           # i will only actually create the file for the optimum case (for all the rest, i just need the name for the dictionary of distances)

            dir="../Results/network_final_schedule_withTeam3_local/"  
            output_file=dir+"Avg_time_evol_Persuasion_training_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_unif_distr_"+str(Niter_training)+"iter_day"+str(cutting_day)+".dat"         





            dir_indiv="../Results/individual_realizations_persuasion/Train_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_day"+str(cutting_day) 



            for iter in range(int(Niter_training)):

                print "         ",iter
               

                indiv_filename=dir_indiv+"/Indiv_realiz_train_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_day"+str(cutting_day)+"iter"+str(iter)+".dat"     




                if os.path.exists(indiv_filename):   # if that file exists

                   result_simu_file= csv.reader(open(indiv_filename, 'rb'), delimiter=' ')
                   
                     # print indiv_filename
                   
                   cont=0
                   time_evol_number_adopters=[]   # for a single realization of the dynamics
                   for row in result_simu_file:   
            ...
            ###########   end of loop over Niter_training
...
# i save just that Average Time evolution file  (i re-read all individual files, and create the average file for the optimum case)
   ...
   output_file_opt=dir+"Avg_time_evol_Persuasion_training_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_unif_distr_"+str(Niter_training)+"iter_day"+str(cutting_day)+".dat"         

   file_opt = open(output_file_opt,'wt')        
   for i in range(len(time_evol_number_adopters)):  #time step by time step
      list_fixed_t=[]
      for iteracion in range (Niter_training): #loop over all independent iter of the process
         list_fixed_t.append(time_evol_number_adopters_ITER[iteracion][i])  # i collect all values for the same t, different iter  

         print >> file_opt, list_t[i],numpy.mean(list_fixed_t),numpy.std(list_fixed_t), alpha_F,damping,mutual_encouragement       
   file_opt.close()



   print "\n printed out optimum testing file:", file_opt
...
if __name__ == '__main__':
    if len(sys.argv) > 2:
        ...
        main(graph_filename,Niter_train)
    else:
        print "Usage: python script.py path/network.gml  Number_existing_indiv_training_files"
