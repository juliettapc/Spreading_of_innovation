...
def main(graph_name):
   ...
   output_file3="../Results/weight_shifts/Landscape_parameters_persuasion_"+str(Niter)+"iter_alphaA_eq_alphaF.dat" 
...
#OJO!!! NECESITO DOS DECIMALES SIEMPRE, PARA QUE CUADRE CON EL NOMBRE DE LOS SUB-DIRECTORIOS DONDE LO GUARDO
   ...
   alpha_F_min=0.00   #0.15   # alpha=0: nobody changes their mind
   alpha_F_max=0.90    #0.351
   ...
   max_mutual_encouragement=0.50   # 0.51   # KEEP THIS FIXED VALUES FOR NOW
   delta_mutual_encouragement=0.10
   
   
   threshold_min=0.0   #0.50  # larger than, to be an Adopte
   threshold_max=0.9  # 0.51    # KEEP THIS FIXED VALUES FOR NOW
 ...
 #   dir="../Results/weight_shifts/persuasion/alpha"+str(alpha_F)+"_damping"+str(damping)+"/" 
...
#    print "one-day weekend", G.node[n]['label'],G.node[n]['shift_lenght']
   ...
   compare_real_evol_vs_simus_to_be_called.pick_minimum_same_end(dict_filenames_tot_distance,"Persuasion_weight",all_team,Niter,None)  #las argument, cutting day (it doesnt apply)
