...
def main(graph_name):
    ...
    while alpha_F<= alpha_F_max:
        ...
        dir="../Results/"   
        output_file2=dir+"Final_number_Adopters_Persuasion_alpha"+str(alpha_F)+"_dampingNonUnif_mutual_encourg"+str(mutual_encouragement)+"_unif_distr_"+str(Niter)+"iter_2012.dat"    
        ...
        for num_initial_shifts in range(tot_num_shifts):                                 
            ...
            output_file=dir+"Time_evolutions_Persuasion_alpha"+str(alpha_F)+"_dampingNonUnif_mutual_encourg"+str(mutual_encouragement)+"_num_ini_shifts"+str(num_initial_shifts)+"_unif_distr_"+str(Niter)+"iter_2012.dat"        
                            ...
                            if flag_possible_persuasion==1:
                                ...
                                for pair in pairs:
                                    ...
                                    if G.node[doctor1]['status'] != G.node[doctor2]['status']:  # if they think differently, 
                                        ...
                                        persuasion(G,doctor1,doctor2,alpha_A,alpha_F,threshold)                                   
            ...
            for s in range(tot_num_shifts):           
                ...
                print >> file, s,numpy.mean(list_fixed_t), alpha_F,mutual_encouragement                             
            file.close()



            file2 = open(output_file2,'at')   
            print >> file2,  num_initial_shifts, numpy.mean(list_final_number_Adopters_fixed_num_shift),alpha_F,mutual_encouragement
            file2.close()
...
def persuasion(G,doctor1,doctor2,alpha_A,alpha_F,threshold):
    ...
    if G.node[doctor1]["adoption_vector"]>=threshold or G.node[doctor2]["adoption_vector"]>=threshold :  # only if at least one doctor is Adopter
        ...
        if (G.node[doctor1]['type']=="A") and (G.node[doctor2]['type']=="F"):
            ...
            if float(delta_AF)>=0.0:
                G.node[doctor1]["adoption_vector"]=G.node[doctor1]["adoption_vector"]-delta_AF*alpha_A*damping(G.node[doctor1]["adoption_vector"])  
                G.node[doctor2]["adoption_vector"]=G.node[doctor2]["adoption_vector"]+delta_AF*alpha_F*damping(G.node[doctor2]["adoption_vector"])   
            else:
                G.node[doctor1]["adoption_vector"]=G.node[doctor1]["adoption_vector"]-delta_AF*alpha_A*damping(G.node[doctor1]["adoption_vector"])  
                G.node[doctor2]["adoption_vector"]=G.node[doctor2]["adoption_vector"]+delta_AF*alpha_F*damping(G.node[doctor2]["adoption_vector"])   
            ...
            if float(delta_FA)>=0.0:
                G.node[doctor1]["adoption_vector"]=G.node[doctor1]["adoption_vector"]-delta_FA*alpha_F*damping(G.node[doctor1]["adoption_vector"])  
                G.node[doctor2]["adoption_vector"]=G.node[doctor2]["adoption_vector"]+delta_FA*alpha_A*damping(G.node[doctor2]["adoption_vector"])   
            else:
                G.node[doctor1]["adoption_vector"]=G.node[doctor1]["adoption_vector"]-delta_FA*alpha_F*damping(G.node[doctor1]["adoption_vector"])  
                G.node[doctor2]["adoption_vector"]=G.node[doctor2]["adoption_vector"]+delta_FA*alpha_A*damping(G.node[doctor2]["adoption_vector"])  
            ...
            if float(delta_FF)>=0.0:
                G.node[doctor1]["adoption_vector"]=G.node[doctor1]["adoption_vector"]-delta_FF*alpha_F*damping(G.node[doctor1]["adoption_vector"])  
                G.node[doctor2]["adoption_vector"]=G.node[doctor2]["adoption_vector"]+delta_FF*alpha_F*damping(G.node[doctor2]["adoption_vector"])
            else:
                G.node[doctor1]["adoption_vector"]=G.node[doctor1]["adoption_vector"]-delta_FF*alpha_F*damping(G.node[doctor1]["adoption_vector"])  
                G.node[doctor2]["adoption_vector"]=G.node[doctor2]["adoption_vector"]+delta_FF*alpha_F*damping(G.node[doctor2]["adoption_vector"])
            ...
            if float(delta_AA)>=0.0:
                G.node[doctor1]["adoption_vector"]=G.node[doctor1]["adoption_vector"]-delta_AA*alpha_A*damping(G.node[doctor1]["adoption_vector"])  
                G.node[doctor2]["adoption_vector"]=G.node[doctor2]["adoption_vector"]+delta_AA*alpha_A*damping(G.node[doctor2]["adoption_vector"])
            else:
                G.node[doctor1]["adoption_vector"]=G.node[doctor1]["adoption_vector"]-delta_AA*alpha_A*damping(G.node[doctor1]["adoption_vector"])  
                G.node[doctor2]["adoption_vector"]=G.node[doctor2]["adoption_vector"]+delta_AA*alpha_A*damping(G.node[doctor2]["adoption_vector"])
...
def damping (x):  #the closer u are to the extremes, the harder it is to move. 
    return 4.0*x*(1.0-x)
