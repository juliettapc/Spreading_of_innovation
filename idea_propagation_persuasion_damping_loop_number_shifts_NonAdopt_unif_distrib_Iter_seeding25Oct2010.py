...
def main(graph_name):
    ...
    while alpha_F<= alpha_F_max:
            ...
            dir="../Results/"   
            output_file2=dir+"Final_number_Adopters_Persuasion_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_unif_distr_"+str(Niter)+"iter_2010_seed25Oct.dat"    
            ...
            output_file=dir+"Time_evolutions_Persuasion_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_unif_distr_"+str(Niter)+"iter_2010_seed25Oct.dat"        
            ...
            list_lists_t_evolution_adopters_on_call=[]    
            for iter in range(Niter):

                num_adopters , list_order_seed_shift ,max_shift= set_ic(G,threshold)

                list_single_t_evolution=[]                
                list_single_t_evolution.append(num_adopters)

                list_t_evolution_adopters_on_call=[]                
                list_t_evolution_adopters_on_call.append(2)
               ...
               # the dynamics starts:                 
                ...
                while t<= max_shift:  # loop over shifts, in chronological order   
                    ...
                    for n in G.nodes():              
                        ...
                        if G.node[n]['type']=="shift" and G.node[n]['order']==t: #count # adopters on call
                            num_adopters_on_call=0
                            for node in G.neighbors(n):
                                if  G.node[node]['type']!="shift" and G.node[node]['status']=="Adopter":
                                    num_adopters_on_call +=1
                                    #print G.node[node]['type'] , G.node[node]['label']
        
                    #print t, "# adopters on call:",num_adopters_on_call,"\n\n"

                   
                    list_single_t_evolution.append(float(len(list_Adopters)))
         
                    list_t_evolution_adopters_on_call.append(num_adopters_on_call)
                ...
                list_lists_t_evolution_adopters_on_call.append(list_t_evolution_adopters_on_call)

            #end loop over Niter
            file = open(output_file,'at')        
            for s in range(len(list_single_t_evolution)):           
                list_fixed_t=[]
                list_fixed_t_adopters_on_call=[]
                for iter in range (Niter):
                    list_fixed_t.append(list_lists_t_evolutions[iter][s])        
                    list_fixed_t_adopters_on_call.append(list_lists_t_evolution_adopters_on_call[iter][s])        

                print >> file, s,numpy.mean(list_fixed_t),numpy.std(list_fixed_t),numpy.mean(list_fixed_t_adopters_on_call),numpy.std(list_fixed_t_adopters_on_call), alpha_F,damping,mutual_encouragement                             
            file.close()



            file2 = open(output_file2,'at')   
            print >> file2,   numpy.mean(list_final_number_Adopters_fixed_num_shift),numpy.std(list_fixed_t),alpha_F,damping,mutual_encouragement
            file2.close()
...
def set_ic(G,threshold):
        ...
        for n in G.nodes():              
            if G.node[n]['type']=="shift":
                if n not in list_s:
                    list_s.append(n)                        
                if "1/04/10" in G.node[n]['label']: # actual seeding date                          
        ...
        for seed in list_seed_shift: # i can infect initially more than one shift
            for doctor in G.neighbors(seed):
                ...
                if G.node[doctor]["label"]=="Moore" or G.node[doctor]["label"]=="Budinger" or G.node[doctor]["label"]=="Go":  # one entire team and one doctor in the other team
