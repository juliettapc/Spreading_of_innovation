...
#../Results/weight_shifts/infection/Average_time_evolution_Infection_training_p0.8_Immune0.3_1000iter_2012_avg_ic_day125.dat ESTOS VALORES SON EL OPTIMUM FIT THE 152-DIAS
   ...
   while prob_Immune<= prob_Immune_max:
     ...
     while prob_infection<= prob_max:
       ...
       while dose <= dose_max:
        ...
        if for_testing_fixed_set=="YES":
           output_file2=dir+"Average_time_evolution_Infection_memory_distrib_train_test_p"+str(prob_infection)+"_"+"Immune"+str(prob_Immune)+"_"+str(Niter)+"iter_2012.dat"

        else:
           output_file2=dir+"Average_time_evolution_Infection_memory_distrib_p"+str(prob_infection)+"_"+"Immune"+str(prob_Immune)+"_"+str(Niter)+"iter_2012.dat"
     ...
     #   list_abs_dist_at_cutting_day=[]
        ...
        if  envelopes=="YES":
           calculate_envelope_set_curves.calculate_envelope(list_lists_t_evolutions,percent_envelope,"Infection_memory_distrib",[prob_infection,prob_Immune])
        ...
        if for_testing_fixed_set=="YES":
           ...
           histogram_filename="../Results/weight_shifts/histogr_raw_distances_ending_test_train_infection_memory_distrib_p"+str(prob_infection)+"_"+"Immune"+str(prob_Immune)+"_"+str(Niter)+"iter_day"+str(cutting_day)+".dat"
           histograma_gral_negv_posit.histograma(list_dist_at_ending_point_fixed_parameters,histogram_filename)


     

           output_file10="../Results/weight_shifts/Summary_results_training_segment_infection_memory_distrib_p"+str(prob_infection)+"_"+"Immune"+str(prob_Immune)+"_"+str(Niter)+"iter_day"+str(cutting_day)+".dat"          
   ...
   if for_testing_fixed_set=="NO":   # only if i am exploring the whole landscape, i need to call this function, otherwise, i already know the optimum
      compare_real_evol_vs_simus_to_be_called.pick_minimum_same_end(dict_filenames_tot_distance,"Infection_memory_distrib_training",all_team,Niter,None)  # last argument doesnt apply (cutting day)
