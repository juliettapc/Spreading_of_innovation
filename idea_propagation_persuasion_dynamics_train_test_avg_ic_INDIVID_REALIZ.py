...
def main(graph_name,Niter_train,starting_index):
   ...
   Niter_training=int(Niter_train)
   Niter_testing= int(Niter_train)
...
#../Results/network_final_schedule_withTeam3/Time_evolutions_Persuasion_alpha0.2_damping0.0_mutual_encourg0.7_threshold0.4_unif_distr_50iter_2012_seed31Oct_finalnetwork.dat
   ...
   alpha_F_max=0.9 #AVOID 1.0 OR THE DYNAMICS GETS TOTALLY STUCK AND CANT PREDICT SHIT!
   delta_alpha_F=0.1
   ...
   threshold_max=0.9    #AVOID 1.0 OR THE DYNAMICS GETS TOTALLY STUCK AND CANT PREDICT SHIT!
   delta_threshold=0.1
      ...
      while alpha_F<= alpha_F_max:            # i explore all the parameter space, and create a file per each set of values
        ...
        while  mutual_encouragement <= max_mutual_encouragement:
          ...
          while   damping <= max_damping:
            ...
            dir_indiv="../Results/individual_realizations_persuasion/Train_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_day"+str(cutting_day)   # i create a directory  (one directory per parameter point in the 4D parameter space, containing all indiv. realizations)
            if not os.path.exists(dir_indiv):
               os.makedirs(dir_indiv)


               
            flag_existing_file=1   # i look for the next file index (so i dont overwrite existing individual iteration files)
            index=0
            while (flag_existing_file):
      
               if os.path.exists(dir_indiv+"/Indiv_realiz_train_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_day"+str(cutting_day)+"iter"+str(index)+".dat" ):   # returns true/false  (if the file exists, i enter the if loop)
                  #print "file",index, "exists"
                  index+=1

               else:
                  flag_existing_file=0
                  print "next file index:", index   # this will be the index for the next individual file that i create
...
#THIS WILL GO TO AN INDEPENDENT ANALYSIS CODE
          #  dir="../Results/network_final_schedule_withTeam3_local/"  
           # output_file=dir+"Time_evolutions_Persuasion_training_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_unif_distr_"+str(Niter_training)+"iter_2012_seed31Oct_finalnetwork_day"+str(cutting_day)+".dat"         
            ...
            for iter in range(Niter_training):
                iter_corrected=iter+int(starting_index)
              # iter = iter + starting_index  # just in case some previous realizations exist, so i dont overwrite them
              
                print "         ",iter, iter_corrected



                indiv_filename=dir_indiv+"/Indiv_realiz_train_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_day"+str(cutting_day)+"iter"+str(iter_corrected)+".dat"         
                file_indiv = open(indiv_filename,'wt')   # save indiv. realization 
               



                indiv_dict_filename=dir_indiv+"/Dict_States_nodes_indiv_realiz_train_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_day"+str(cutting_day)+"iter"+str(iter_corrected)+".dat"         
                file_indiv_dict = open(indiv_dict_filename,'wt')   # save state all nodes at cutting day
               ...
               ########### the dynamics starts:                 
                ...
                while t< cutting_day:  # loop over shifts, in chronological order  (the order is the day index since seeding_day) 
                    ...
                    print >> file_indiv, t, float(len(list_Adopters))
                    t+=1
                ...
                for n in G.nodes():            
                   if   G.node[n]['type']!="shift":                  
                      ...
                      print >> file_indiv_dict,G.node[n]["label"], G.node[n]["status"]
...
#            file.close()
   ...
   list_order_dict=  compare_real_evol_vs_simus_to_be_called.pick_minimum_same_end(dict_filenames_tot_distance,"Persuasion_training",all_team,Niter_training,cutting_day)
...
#./Results/network_final_schedule_withTeam3_local/Time_evolutions_Persuasion_alpha0.4_damping0.4_mutual_encourg0.6_threshold0.5_unif_distr_2iter_2012_seed31Oct_finalnetwork.dat
   ...
   print "alpha=", alpha_F, " damping=",damping," mutual encourag=",mutual_encouragement," threshold",threshold
...
#list_sorted_dict=[(u'Weiss', 5.0), (u'Wunderink', 5.0), (u'Keller', 4.0), (u'Go', 3.0), (u'Cuttica', 3.0), (u'Rosario', 2.0), (u'Radigan', 2.0), (u'Smith', 2.0), (u'RosenbergN', 2.0), (u'Gillespie', 1.0), (u'Osher', 1.0), (u'Mutlu', 1.0), (u'Dematte', 1.0), (u'Hawkins', 1.0), (u'Gates', 1.0)]
   ...
   for iter in range(Niter_testing):
      ...
      for node in G.nodes():
         if   G.node[node]['type']!="shift":
            G.node[node]["adoption_vector"]=random.random()*threshold  #values from [0,threshold)
            ...
            if label in lista_avg_Adopters:    
               ...
               G.node[node]["adoption_vector"]=random.random()*(1.0-threshold)+threshold  #values from (threshold,1]
   ...
   histograma_gral_negv_posit.histograma(list_dist_at_ending_point_fixed_parameters,"../Results/histogr_raw_distances_ending_test_train_persuasion_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_"+str(Niter_training)+"iter_avg_ic_day"+str(cutting_day)+".dat" ) 




   output_file8="../Results/List_tot_distances_training_segment_persuasion_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_"+str(Niter_training)+"iter_avg_ic_day"+str(cutting_day)+".dat"       
   ...
   output_file9="../Results/List_distances_ending_training_segment_persuasion_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_"+str(Niter_training)+"iter_avg_ic_day"+str(cutting_day)+".dat"         
   ...
   output_file=dir+"Time_evolutions_Persuasion_testing_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_unif_distr_"+str(Niter_training)+"iter_2012_seed31Oct_finalnetwork_avg_ic_day"+str(cutting_day)+".dat"         
   file = open(output_file,'wt')       
   ...
   output_file10="../Results/Summary_results_train_test_persuasion_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_"+str(Niter_training)+"iter_avg_ic_day"+str(cutting_day)+".dat"         
   file10 = open(output_file10,'wt')    

   print >> file10, "Summary results from train-testing persuasion with",Niter_training, Niter_testing, "iter (respectively), using the avg of the cutting points as IC, and with values for the parameters:  alpha ",alpha_F," damping: ",damping," mutual_encourg: ",mutual_encouragement," threshold:",threshold
...
if __name__ == '__main__':
    if len(sys.argv) > 3:
        ...
        Niter_train=sys.argv[2]
        starting_index=sys.argv[3]

   
        main(graph_filename,Niter_train,starting_index)
    else:
        print "Usage: python script.py path/network.gml  Niter_training  staring_iter_index"
