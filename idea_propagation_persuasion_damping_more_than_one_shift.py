...
def main(graph_name):
    ...
    alpha_F_max=0.21
    delta_alpha_F=0.05
    damping=0.50  #its harder to go back from YES to NO again. =1 means no effect, =0.5 half the movement from Y->N than the other way around


    mutual_encouragement=0.1  # when two Adopters meet, they encourage each other


    num_initial_shifts=2
    ...
    while alpha_F<= alpha_F_max:
        ...
        dir="../Results/"   
        output_file=dir+"Time_evolutions_Persuasion_alpha"+str(alpha_F)+"_damping"+str(damping)+"num_shifts"+str(num_initial_shifts)+".dat"    
        file = open(output_file,'wt')    
        file.close()


       # output_file2=dir+"Average_time_evolution_Persuasion_alpha"+str(alpha_F)+"_damping"+str(damping)+"_"+str(Niter)+"iter.dat"
        ...
        for iter in range(Niter):            
            ...
            for seed in seed_shift:
                ...
                print >> file,G.node[seed]['order'], float(len(list_Adopters))  # /(len(list_A)+len(list_F)),list_Adopters
                file.close()
        
            print  "# adopters at t 0", len(list_Adopters),list_Adopters

  
# the dynamics starts: 
            list_single_t_evolution=[]
            t=int(min(seed_shift))#G.node[seed_shift]['order']
                                  ...
                                  #  print "possible persuasion:", G.node[doctor1]['label'],G.node[doctor1]["adoption_vector"], G.node[doctor1]['status'],"--",G.node[doctor2]['label'],G.node[doctor2]["adoption_vector"], G.node[doctor2]['status'],
                                    ...
                                    if (G.node[doctor1]['type']=="A") and (G.node[doctor2]['type']=="F"):
                                        ...
                                        if float(delta_AF)>=0.0:
                                            G.node[doctor1]["adoption_vector"]=G.node[doctor1]["adoption_vector"]-delta_AF*alpha_A*damping
...
####################################
                                    if float(G.node[doctor1]["adoption_vector"])>=threshold: 
                                        ...
                                        if G.node[doctor1]["label"] not in list_Adopters:
                                        ...
                                        try:
                                            list_Adopters.remove(G.node[doctor1]["label"])
                                    ...
                                    if float(G.node[doctor2]["adoption_vector"])>=threshold:
                                        ...
                                        if G.node[doctor2]["label"] not in list_Adopters:
                                        ...
                                        try:
                                            list_Adopters.remove(G.node[doctor2]["label"])  
