...
def main ():
  ...
  # result_actual_file= csv.reader(open(filename_actual_evol, 'rb'), delimiter=',')
    ...
    for line in lista_lines:  
        ...
        listY1.append(float(items[13]))  # Number successful of interventions per simulation  (tot)
        listY2.append(float(items[14]))   #(only on att)
        listY3.append(float(items[15]))    #(only on f)
  ...
  #  print listX, listY
    ...
    yTitle="Fraction interventions"
    title="" #empty
    subtitle="" #empty
    filename="../Results/weight_shifts/figures_pygrace/Fig_fracion_interventions_vs_bump_window"+str(window)+".agr"
