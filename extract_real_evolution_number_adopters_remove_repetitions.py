...
##  OJO!!!!!! REVISAR EL NUMERO DE ESTA COLUMNA PQ PUEDE SER DISTINTO DE UN ARCHIVO DE DATOS A OTROS       
    ...
    results= csv.reader(open(data_filename, 'r'), delimiter=',')   #remember!!! every time that i wanna access the csv file, i must read it AGAIN!!
    reader = csv.DictReader(open(data_filename))
    csv_header = reader.fieldnames
    ...
    for row in results:       
        if row != csv_header:  # i exclude the first row
            ...
            if (date != previous_date):# this condition is enough to remove duplicated order!  
