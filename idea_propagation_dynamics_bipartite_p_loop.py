...
def main(graph_name):
    ...
    while prob_infection<= prob_max:
        ...
        dir="../Results/"   
        output_file=dir+"Time_evolutions_Infection_p"+str(prob_infection)+"_2012.dat"    
        file = open(output_file,'wt')    
        file.close()


        output_file2=dir+"Average_time_evolution_Infection_p"+str(prob_infection)+"_"+str(Niter)+"iter_2012.dat"
   ...
   # seed_shift=random.choice(list_s)
    #G.node[seed_shift]["status"]="I"  # Infected
      ...
      #  print "\nt:",G.node[seed_shift]['order'], "I/N:",float(len(list_I))/(len(list_A)+len(list_F)),"list I:",list_I
            ...
            print >> file,G.node[seed_shift]['order'], float(len(list_I))/(len(list_A)+len(list_F)),list_I
            file.close()
...
# the dynamics starts: 
          ...
          #  print "\nt:",t, "I/N:",float(len(list_I))/(len(list_A)+len(list_F)),"list I:",list_I
                ...
                file = open(output_file,'at')                       
                print >> file,t, float(len(list_I))/(len(list_A)+len(list_F)),list_I
                file.close()
