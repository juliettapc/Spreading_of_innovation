...
#../Results/network_final_schedule_withTeam3/Time_evolutions_Persuasion_alpha0.2_damping0.0_mutual_encourg0.7_threshold0.4_unif_distr_50iter_2012_seed31Oct_finalnetwork.dat
      ...
      while alpha_F<= alpha_F_max:            # i explore all the parameter space, and create a file per each set of values
        ...
        while  mutual_encouragement <= max_mutual_encouragement:
          ...
          while   damping <= max_damping:
            ...
            output_file=dir+"Time_evolutions_Persuasion_training_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_unif_distr_"+str(Niter_training)+"iter_2012_seed31Oct_finalnetwork.dat"        
   ...
   list_order_dict=  compare_real_evol_vs_simus_to_be_called.pick_minimum_same_end(dict_filenames_tot_distance,"Persuasion_training",all_team,Niter_training)
...
#./Results/network_final_schedule_withTeam3_local/Time_evolutions_Persuasion_alpha0.4_damping0.4_mutual_encourg0.6_threshold0.5_unif_distr_2iter_2012_seed31Oct_finalnetwork.dat
   ...
   threshold=float(list_order_dict[0][0].split("_threshold")[1][0:3])
...
#   raw_input()
   ...
   for iter in range(Niter_testing):
      ...
      for node in G.nodes():
         if   G.node[node]['type']!="shift":
            ...
            G.node[node]["status"]="NonAdopter"  # initially non-Adopters

            label=G.node[node]['label']
               
            G.node[node]["status"]=dict_dr_status_current_iter[label]  # i use as ic the status of all nodes at the cutting point
            if G.node[node]["status"]=="Adopter":
   ...
   histograma_gral_negv_posit.histograma(list_dist_at_ending_point_fixed_parameters,"../Results/histogr_raw_distances_ending_test_train_persuasion_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_"+str(Niter_training)+"iter_same_end_point.dat"  )      



   output_file8="../Results/List_tot_distances_training_segment_persuasion_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_"+str(Niter_training)+"iter_same_end_point.dat"        
   ...
   output_file9="../Results/List_distances_ending_training_segment_persuasion_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_"+str(Niter_training)+"iter_same_end_point.dat"        
   ...
   output_file=dir+"Time_evolutions_Persuasion_testing_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_unif_distr_"+str(Niter_training)+"iter_2012_seed31Oct_finalnetwork_same_end_point.dat"        
   file = open(output_file,'wt')       
   ...
   output_file10="../Results/Summary_results_train_test_persuasion_alpha"+str(alpha_F)+"_damping"+str(damping)+"_mutual_encourg"+str(mutual_encouragement)+"_threshold"+str(threshold)+"_"+str(Niter_training)+"iter_same_end_point.dat"        
   file10 = open(output_file10,'wt')    

   print >> file10, "Summary results from train-testing persuasion with",Niter_training, Niter_testing, "iter (respectively), using all the individual cutting points as IC, and with values for the parameters:  alpha ",alpha_F," damping: ",damping," mutual_encourg: ",mutual_encouragement," threshold:",threshold
